...
const CreateTaskModal = ({
  ...
  const createTask = async () => {
    ...
    try {
      ...
      if (
        ...
        err.response.data.message.includes("Description")
      )
        setError("Summary must be less than or equal to 1000 characters.");
      else if (
        err.response.status === 400 &&
        err.response.data.message.includes("Deadline")
      )
  ...
  const fetchAllProjects = async () => {
    try {
      ...
      // When user presses 'Create Task' in project page
      if (projectId !== null) {
        const currentProjectDetails: IProject = data.filter(
          (project: IProject) => project.id === parseInt(projectId)
        )[0];
  ...
  const changeProject = (selectProjectId: number) => {
    const currentProjectDetails: IProject = userProjects.filter(
      (project: IProject) => project.id === selectProjectId
    )[0];
    setSelectedProject(currentProjectDetails);
    setCurrProjectId(currentProjectDetails.id.toString());
  };
  ...
  return (
    <Modal open={isOpen} onClose={handleClose}>
      {userProjects.length === 0 ? (
        <ModalContainer>
          <EmptyModal>
            <h1>Please create or join a project</h1>
          </EmptyModal>
        </ModalContainer>
      ) : (
        <ModalContainer>
          ...
          <ModalBody>
            ...
            <BottomContainer>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(e: SelectChangeEvent) => {
                    changeProject(parseInt(e.target.value));
                  }}
                  sx={{ width: "100%" }}
                >
              ...
              <div>
                ...
                <Slider
                  ...
                  onChange={(e, val) =>
                    setTaskDetails({ ...taskDetails, points: val as number })
                  }
                />
              </div>
            </BottomContainer>
            ...
            <ButtonsContainer>
              ...
              <CreateButton variant="contained" onClick={createTask}>
                Create
              </CreateButton>
            </ButtonsContainer>
          </ModalBody>
